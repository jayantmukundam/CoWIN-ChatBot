...
def formatRecvdData(data_recvd):
	...
	if not data_recvd:
		return data_recvd
	print("Hello"+data_recvd)
	# index=-1
	index=data_recvd.find(":")+1
	if index!=-1:
		if data_recvd[index+1]!='':
			cmmd=data_recvd[0:index+1]
			option=dict(data_recvd[index:])
			print(option)
			print("hi")
	return data_recvd
...
if __name__ == '__main__':
	...
	if server_socket != None:
		while True:
			...
			# data_recvd=formatRecvdData(data_recvd)
			print(data_recvd)
